...
let BoundingBoox = require('./BoundingBoox.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class BoundingBooxes {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.bounding_boxes.forEach((val) => {
      bufferOffset = BoundingBoox.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new BoundingBooxes(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.bounding_boxes[i] = BoundingBoox.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.bounding_boxes.forEach((val) => {
      length += BoundingBoox.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'centernet_ros/BoundingBooxes';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new BoundingBooxes(null);
    ...
    if (msg.bounding_boxes !== undefined) {
      ...
      for (let i = 0; i < resolved.bounding_boxes.length; ++i) {
        resolved.bounding_boxes[i] = BoundingBoox.Resolve(msg.bounding_boxes[i]);
      }
    }
    else {
